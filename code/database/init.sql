...
CREATE TABLE `skill_order` (
    ...
    `appointment_time` VARCHAR(100) DEFAULT NULL COMMENT '预约时间',
    `appointment_location` VARCHAR(200) DEFAULT NULL COMMENT '预约地点',
    `plan` TEXT DEFAULT NULL COMMENT '计划安排',
...
CREATE TABLE `appeal` (
    ...
    `evidence_images` TEXT DEFAULT NULL COMMENT '证据图片URL列表，JSON数组',
...
-- 插入测试用户 (密码: 123456)
INSERT INTO user (id, username, password, email, role, balance, bio) VALUES
(2, 'testuser', '$2a$10$nt81daPSNAW0rfMR72Z8XOJb6PfELzUzwApfiIa3Yhb0drL2Y8pjG', 'test@qq.com', 'USER', 100, '测试用户，体验时间银行');

-- 新人礼包流水
INSERT INTO time_transaction (id, user_id, order_id, type, amount, balance_after, remark) VALUES
(1, 2, NULL, 'GIFT', 100, 100, '注册新人礼包：赠送100时间币');
