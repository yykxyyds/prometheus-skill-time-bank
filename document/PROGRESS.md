...
> 当前状态：后端 6 模块全部编译通过，前端 13 个页面完成，认证拦截器已修复（500→401），数据库缺失列已补齐
...
| 订单详情 | `/order/:id` | ✅ | 订单信息+状态时间线+操作按钮+聊天+评价表单，需登录 |
| 申诉提交 | `/appeal/create` | ✅ | 申诉表单（订单ID+原因+证据），需登录 |
...
| 设计模式 | ✅ | 订单状态机（状态模式）+ JWT拦截器（拦截器模式）+ BaseEntity（模板方法模式），已写入详细设计文档第6章 |
| Swagger 接口文档 | ⚠️ | springdoc 依赖已引入，需给 Controller 加注解 |
| Docker 部署 | ❌ | 需写 Dockerfile + docker-compose.yml |
| Redis 缓存 | ❌ | 可缓存技能广场热门列表 |
| 幂等设计 | ❌ | 订单支付接口可加唯一请求号 |
| 单元测试 | ✅ | OrderServiceImpl(30) + UserServiceImpl(29)，共59个测试，纯Mock不启动容器 |
| CI/CD | ❌ | 未配置 |

---

## 五、已修复问题（2026-05-05 本轮）

1. **认证拦截器导致 500 错误** — `UserAuthInterceptor` 只在 `@RequireAuth` 存在时解析 token，但 OrderController/ChatController/SkillController/BountyController 缺少该注解 → userId 为 null → NPE → 500。已修复：改造拦截器始终解析 token，补全注解，gateway 统一注册。
2. **订单列表永远为空** — 后端 `/order/buyer|seller` 返回 `List`（非 `Page`），前端错误使用 `.records` 取值。已修复。
3. **4 张表缺少 `update_time`** — `chat_message`、`bounty_application`、`user_follow`、`notification` 实体继承 `BaseEntity` 但表缺列，导致 SQL 报错。已补全。
4. **补齐缺失页面** — 订单详情+聊天、悬赏管理按钮、申诉提交、评价表单。
...
1. **写详细设计文档** — 含 ER 图、模块结构、接口列表，代码已完成，写起来很快
2. **写需求规格说明 SRS** — 用例图、功能需求、非功能需求
3. **项目任务书** — 格式参考 `提交示例/`
4. **答辩 PPT** — 项目背景 + 技术架构 + 亮点展示
...
12. ~~**单元测试**~~ ✅ 已完成 — Service 层关键逻辑（OrderServiceImpl + UserServiceImpl, 59个测试）

---

## 七、后端 API 清单（供前后端联调参考）
